...
export interface NFLDataProvider { getSeason(): number; getGames(season: number, weekTypes?: WeekType[]): Promise<ProviderGame[]>; }
...
export class EspnNFLProvider implements NFLDataProvider {
  ...
  async getGames(season: number, weekTypes: WeekType[] = ['regular','wildcard','divisional','conference','superbowl']) {
    const all: ProviderGame[] = [];
    const seasonTypes = [
      ...(weekTypes.includes('preseason') ? [1] : []),
      ...(weekTypes.includes('regular') ? [2] : []),
      ...(weekTypes.some(type => ['wildcard','divisional','conference','superbowl'].includes(type)) ? [3] : []),
    ];
    for (const seasonType of seasonTypes) {
      const calendarYears = seasonType === 2 ? [season, season + 1] : [seasonType === 3 ? season + 1 : season];
      for (const calendarYear of calendarYears) {
      const response = await fetch(`${this.baseUrl}/scoreboard?limit=1000&dates=${calendarYear}&seasontype=${seasonType}`, { cache: 'no-store' });
      if (!response.ok) throw new Error(`NFL provider: ${response.status}`);
      const json = await response.json();
      for (const event of json.events || []) {
        if (event.season?.type !== seasonType || event.season?.year !== season) continue;
        ...
        const isSuperBowl = /super bowl/i.test(`${event.name || ''} ${event.shortName || ''}`) || providerWeekNumber === 4 || providerWeekNumber === 5;
        if (seasonType === 3 && providerWeekNumber > 3 && !isSuperBowl) continue;
        const weekNumber = seasonType === 3 ? providerWeekNumber === 1 ? 19 : providerWeekNumber === 2 ? 20 : providerWeekNumber === 3 ? 21 : 22 : providerWeekNumber;
        let weekType: WeekType = seasonType === 1 ? 'preseason' : 'regular';
        ...
        if (!weekTypes.includes(weekType)) continue;
        ...
        const label = seasonType === 1 ? `Pretemporada · Semana ${providerWeekNumber}` : seasonType === 3 ? `Semana ${weekNumber} · ${playoffLabel}` : event.week?.text || `Semana ${weekNumber}`;
        all.push({ externalId: event.id, season, week: weekNumber, weekType, weekLabel: label, kickoffAt: event.date,
    ...
    return [...new Map(all.map(game => [game.externalId, game])).values()];
  }
}
