...
export async function getAppData(seasonId?: string, weekId?: string) {
  ...
  const winStatuses = calculateWeeklyWinStatuses(marked, games || []);
  ...
  return { user, seasons, season, weeks, week, games, picks:publicPicks, players, tiebreakers:publicTies, weekly:publicWeekly, seasonStandings, adminUsers, now:new Date().toISOString() };
}
